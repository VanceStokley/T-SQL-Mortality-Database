IF OBJECT_ID('mcod.Raw_2019','U') IS NOT NULL DROP TABLE mcod.Raw_2019;
IF OBJECT_ID('mcod.Raw_2019_stage','U') IS NOT NULL DROP TABLE mcod.Raw_2019_stage;

CREATE TABLE mcod.Raw_2019_stage ( RawLine VARCHAR(490) NOT NULL );

BULK INSERT mcod.Raw_2019_stage
FROM 'C:\nvss\2019\mort2019us.dat'
WITH (FORMATFILE='C:\nvss\fmt_mort_490.xml', CODEPAGE='RAW', TABLOCK, MAXERRORS=0);

ALTER TABLE mcod.Raw_2019_stage ADD RawID BIGINT IDENTITY(1,1) NOT NULL;
EXEC sp_rename 'mcod.Raw_2019_stage','Raw_2019','OBJECT';

DECLARE @Out19 TABLE (RowNum INT IDENTITY(1,1), MortalityID BIGINT);
...
OUTPUT inserted.MortalityID INTO @Out19(MortalityID)
SELECT
  2019,
  ...
  NULL, NULL
FROM mcod.Raw_2019 r
ORDER BY r.RawID;

DECLARE @Map19 TABLE (MortalityID BIGINT, RawID BIGINT PRIMARY KEY);
WITH r AS (SELECT RawID, ROW_NUMBER() OVER (ORDER BY RawID) RN FROM mcod.Raw_2019)
INSERT @Map19 SELECT o.MortalityID, r.RawID FROM @Out19 o JOIN r ON r.RN=o.RowNum;
...
FROM @Map19 m JOIN mcod.Raw_2019 x ON x.RawID=m.RawID JOIN Slots s ON 1=1
