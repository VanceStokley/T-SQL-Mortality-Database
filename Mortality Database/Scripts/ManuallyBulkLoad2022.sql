IF OBJECT_ID('mcod.Raw_2022','U') IS NOT NULL DROP TABLE mcod.Raw_2022;
IF OBJECT_ID('mcod.Raw_2022_stage','U') IS NOT NULL DROP TABLE mcod.Raw_2022_stage;

CREATE TABLE mcod.Raw_2022_stage ( RawLine VARCHAR(817) NOT NULL );

BULK INSERT mcod.Raw_2022_stage
FROM 'C:\nvss\2022\mort2022us.dat'
WITH (FORMATFILE='C:\nvss\fmt_mort_817.xml', CODEPAGE='RAW', TABLOCK, MAXERRORS=0);

ALTER TABLE mcod.Raw_2022_stage ADD RawID BIGINT IDENTITY(1,1) NOT NULL;
EXEC sp_rename 'mcod.Raw_2022_stage','Raw_2022','OBJECT';

DECLARE @Out22 TABLE (RowNum INT IDENTITY(1,1), MortalityID BIGINT);
...
OUTPUT inserted.MortalityID INTO @Out22(MortalityID)
SELECT
  2022,
  ...
  SUBSTRING(r.RawLine,450,1),     -- RaceRecode6 (2022+)
  SUBSTRING(r.RawLine,484,3),
  SUBSTRING(r.RawLine,487,2),     -- HispOrigRaceRecode (2 chars, 2022+)
  SUBSTRING(r.RawLine,489,2),
  SUBSTRING(r.RawLine,806,4), SUBSTRING(r.RawLine,812,4)
FROM mcod.Raw_2022 r
ORDER BY r.RawID;

DECLARE @Map22 TABLE (MortalityID BIGINT, RawID BIGINT PRIMARY KEY);
WITH r AS (SELECT RawID, ROW_NUMBER() OVER (ORDER BY RawID) RN FROM mcod.Raw_2022)
INSERT @Map22 SELECT o.MortalityID, r.RawID FROM @Out22 o JOIN r ON r.RN=o.RowNum;
...
FROM @Map22 m JOIN mcod.Raw_2022 x ON x.RawID=m.RawID JOIN Slots s ON 1=1
