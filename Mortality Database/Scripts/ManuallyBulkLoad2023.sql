IF OBJECT_ID('mcod.Raw_2023','U') IS NOT NULL DROP TABLE mcod.Raw_2023;
IF OBJECT_ID('mcod.Raw_2023_stage','U') IS NOT NULL DROP TABLE mcod.Raw_2023_stage;

CREATE TABLE mcod.Raw_2023_stage ( RawLine VARCHAR(817) NOT NULL );

BULK INSERT mcod.Raw_2023_stage
FROM 'C:\nvss\2023\mort2023us.dat'
WITH (FORMATFILE='C:\nvss\fmt_mort_817.xml', CODEPAGE='RAW', TABLOCK, MAXERRORS=0);

ALTER TABLE mcod.Raw_2023_stage ADD RawID BIGINT IDENTITY(1,1) NOT NULL;
EXEC sp_rename 'mcod.Raw_2023_stage','Raw_2023','OBJECT';

DECLARE @Out23 TABLE (RowNum INT IDENTITY(1,1), MortalityID BIGINT);
...
OUTPUT inserted.MortalityID INTO @Out23(MortalityID)
SELECT
  2023,
  ...
  SUBSTRING(r.RawLine,450,1),    -- RaceRecode6
  SUBSTRING(r.RawLine,484,3),
  SUBSTRING(r.RawLine,487,2),    -- 2-char HispOrigRaceRecode
  SUBSTRING(r.RawLine,489,2),
  SUBSTRING(r.RawLine,806,4), SUBSTRING(r.RawLine,812,4)
FROM mcod.Raw_2023 r
ORDER BY r.RawID;

DECLARE @Map23 TABLE (MortalityID BIGINT, RawID BIGINT PRIMARY KEY);
WITH r AS (SELECT RawID, ROW_NUMBER() OVER (ORDER BY RawID) RN FROM mcod.Raw_2023)
INSERT @Map23 SELECT o.MortalityID, r.RawID FROM @Out23 o JOIN r ON r.RN=o.RowNum;
...
FROM @Map23 m JOIN mcod.Raw_2023 x ON x.RawID=m.RawID JOIN Slots s ON 1=1
