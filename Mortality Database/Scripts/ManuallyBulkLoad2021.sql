IF OBJECT_ID('mcod.Raw_2021','U') IS NOT NULL DROP TABLE mcod.Raw_2021;
IF OBJECT_ID('mcod.Raw_2021_stage','U') IS NOT NULL DROP TABLE mcod.Raw_2021_stage;

CREATE TABLE mcod.Raw_2021_stage ( RawLine VARCHAR(817) NOT NULL );

BULK INSERT mcod.Raw_2021_stage
FROM 'C:\nvss\2021\mort2021us.dat'
WITH (FORMATFILE='C:\nvss\fmt_mort_817.xml', CODEPAGE='RAW', TABLOCK, MAXERRORS=0);

ALTER TABLE mcod.Raw_2021_stage ADD RawID BIGINT IDENTITY(1,1) NOT NULL;
EXEC sp_rename 'mcod.Raw_2021_stage','Raw_2021','OBJECT';

DECLARE @Out21 TABLE (RowNum INT IDENTITY(1,1), MortalityID BIGINT);
...
OUTPUT inserted.MortalityID INTO @Out21(MortalityID)
SELECT
  2021,
...
FROM mcod.Raw_2021 r
ORDER BY r.RawID;

DECLARE @Map21 TABLE (MortalityID BIGINT, RawID BIGINT PRIMARY KEY);
WITH r AS (SELECT RawID, ROW_NUMBER() OVER (ORDER BY RawID) RN FROM mcod.Raw_2021)
INSERT @Map21 SELECT o.MortalityID, r.RawID FROM @Out21 o JOIN r ON r.RN=o.RowNum;
...
FROM @Map21 m JOIN mcod.Raw_2021 x ON x.RawID=m.RawID JOIN Slots s ON 1=1
