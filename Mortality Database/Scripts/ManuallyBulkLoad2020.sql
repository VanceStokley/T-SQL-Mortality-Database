IF OBJECT_ID('mcod.Raw_2020','U') IS NOT NULL DROP TABLE mcod.Raw_2020;
IF OBJECT_ID('mcod.Raw_2020_stage','U') IS NOT NULL DROP TABLE mcod.Raw_2020_stage;

CREATE TABLE mcod.Raw_2020_stage ( RawLine VARCHAR(817) NOT NULL );

BULK INSERT mcod.Raw_2020_stage
FROM 'C:\nvss\2020\mort2020us.dat'
WITH (FORMATFILE='C:\nvss\fmt_mort_817.xml', CODEPAGE='RAW', TABLOCK, MAXERRORS=0);

ALTER TABLE mcod.Raw_2020_stage ADD RawID BIGINT IDENTITY(1,1) NOT NULL;
EXEC sp_rename 'mcod.Raw_2020_stage','Raw_2020','OBJECT';

DECLARE @Out20 TABLE (RowNum INT IDENTITY(1,1), MortalityID BIGINT);
...
OUTPUT inserted.MortalityID INTO @Out20(MortalityID)
SELECT
  2020,
  ...
  SUBSTRING(r.RawLine,448,1),
  CAST(NULL AS VARCHAR(1)),      -- RaceRecode6 not in 2020
  SUBSTRING(r.RawLine,484,3),
  CAST(NULL AS VARCHAR(2)),      -- 2-char HispOrigRaceRecode not in 2020
  SUBSTRING(r.RawLine,489,2),
  SUBSTRING(r.RawLine,806,4), SUBSTRING(r.RawLine,812,4)
FROM mcod.Raw_2020 r
ORDER BY r.RawID;

DECLARE @Map20 TABLE (MortalityID BIGINT, RawID BIGINT PRIMARY KEY);
WITH r AS (SELECT RawID, ROW_NUMBER() OVER (ORDER BY RawID) RN FROM mcod.Raw_2020)
INSERT @Map20 SELECT o.MortalityID, r.RawID FROM @Out20 o JOIN r ON r.RN=o.RowNum;
...
FROM @Map20 m JOIN mcod.Raw_2020 x ON x.RawID=m.RawID JOIN Slots s ON 1=1
